...
const UI = () => {
  const { titleText } = useSiteGlobals();
  const [ infoIsActive, setInfoIsActive ] = useState(false);

  return (
    <>
      <h1 className='select-none fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center p-2 mix-blend-difference text-white'>
        { titleText }
      </h1>
      <button
