				...
				>{showFilters ? 'hide' : 'show'} filters</button>
				{
					<div className='grid grid-cols-2 md:grid-cols-4 gap-4 w-full top-0 bg-white z-10 pt-4'>
						<div className='flex justify-between gap-4'>
							...
							<button onClick={() => setOrderBy('title')}>{orderBy === 'title' ? '–' : '+'}</button>
						</div>
							
						{
							...
							<div className='flex justify-between gap-4'>
								<p>client</p>
								<button onClick={() => setOrderBy('client')}>{orderBy === 'client' ? '–' : '+'}</button>
							</div>
						}
						{
							windowWidth >= 1200 &&
							<div className='flex justify-between gap-4'>
								<p>agency</p>
								<button onClick={() => setOrderBy('agency')}>{orderBy === 'agency' ? '–' : '+'}</button>
							</div>
						}
						<div className='flex justify-between gap-4'>
							<p>year</p>
							<button onClick={() => setOrderBy('year')}>{orderBy === 'year' ? '–' : '+'}</button>
						</div>
					</div>
				}
			</div>
