...
const MuxVideoPlayer = (props) => {
		...
		if (video && value?.hlsUrl) {
			...
			const updateTime = () => {
				...
				if (scrubVisualiser.current) {
					const scrubWidth = (video.currentTime / video.duration) * window.innerWidth;
					scrubVisualiser.current.style.transform = `translateX(${scrubWidth}px)`;
				}
			};
	...
	return (
		<div
			className='w-full h-full fixed top-0 bg-black'
			style={{
		...
		>
			<div className='w-full h-full'>
				...
				/>
				<div className='fixed bottom-0 left-0 w-full h-full z-[2] group flex items-center justify-center transition-opacity duration-300'>
					<input
						type='range'
						className='w-full h-full opacity-0 block fixed top-0 left-0 z-[99]'
						min={0}
						max={duration || 0}
						step={0.01}
						value={currentTime}
			...
			<div
				className='fixed top-0 left-0 w-[1px] h-full bg-white mix-blend-difference opacity-0 transition-opacity duration-300 pointer-events-none'
				style={{
					height: windowHeight + 'px',
					opacity: scrubberIsVisible ? 1 : 0,
					transform: `translateX(${currentTime / duration * windowWidth}px)`,
				}}
			/>
		</div>
	);
