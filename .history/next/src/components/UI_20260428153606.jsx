...
const UI = () => {
  const { titleText, windowWidth, activeItem, infoIsActive, setInfoIsActive, itemInfoIsActive, setItemInfoIsActive, sillyNames, setSillyName, siteGlobals, router } = useSiteGlobals();
  ...
  return (
    <>
      ...
      >
        { router.pathname === '/info' ? 'close' : 'Info' }
      </Link>
      {
        activeItem?.title &&
        infoIsActive === false &&
