...
const ProjectArchiveItemInput = ({ fields, item, index, setValue, removeItem, expandedIndex, setExpandedIndex, onItemExpand, members, renderInput, }) => {
	...
	const renderFieldInput = (fieldName) => {
		...
		console.log('itemMember.members names:', itemMember?.members?.map(m => ({ name: m.name, key: m.key })))
		// const fieldMember = itemMember?.members?.find(m => m.name === fieldName || m.key === fieldName);
		// console.log('fieldMember:', fieldMember);
		// const fieldInput = fieldMember?.field?.input ?? fieldMember?.field;
		// console.log('fieldInput:', fieldInput);
		// return renderInput(fieldInput);
	}
