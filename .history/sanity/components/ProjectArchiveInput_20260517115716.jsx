...
const ProjectArchiveInput = (props) => {
	...
	return (
		<div>
			...
			<div>{showItemsWithoutVideoOrImage ? value?.filter((item) => (!item?.video?._ref && item?.thumbnailTypeIsVideo) || (!item?.image?.asset?._ref && !item?.thumbnailTypeIsVideo)).length : value?.length} items</div>
